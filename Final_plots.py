...
#     if idx == 5:
#         plt.plot(profile[:, 0], profile[:, 2], label=f'After reshaping - Profile {idx}', color='black', linestyle='-', linewidth=0.7)
# #    else:
#        plt.plot(profile[:, 0], profile[:, 2], label=f'After reshaping - Profile {idx}', color='black', linestyle='-', linewidth=0.2)
    ...
    Sd_values.append(Sd)
    plt.fill_between(profile_filtered[:, 0], profile_filtered[:, 2], profile_interp, where=(profile_interp > profile_filtered[:, 2]), color='orange', alpha=0.5, label='Erodert areal ($A_e$) [$m^2$]' if idx == 1 else "")
...
plt.figure(figsize=(10, 6))
plt.plot(mid_profile[:, 0], mid_profile[:, 2], label=f'Midterste tverrsnittsprofil i settet- Profil {mid_idx}', color='red', linestyle='-', linewidth=0.5)
